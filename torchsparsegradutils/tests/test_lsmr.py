...
def lowerBidiagonalMatrix(m, n, dtype, device):
    if m <= n:
        ...
        data = torch.hstack((torch.arange(1, m + 1, dtype=dtype, device=device), torch.arange(1, m, dtype=dtype, device=device)))
    else:
        ...
        idx = torch.vstack((row, col))
        data = torch.hstack((torch.arange(1, n + 1, dtype=dtype, device=device), torch.arange(1, n + 1, dtype=dtype, device=device)))
...
def lsmrtest(m, n, damp, dtype, device):
    ...
    Afun = A.matmul
    b = Afun(xtrue)
    atol = 1e-7; btol = 1e-7; conlim = 1e10; itnlim = 10 * n
...
def test_normar(device):
    ...
    A = lowerBidiagonalMatrix(m,n,dtype,device)
    xtrue = torch.arange(n,0,-1, dtype=dtype, device=device)
    Afun = A.matmul; Arfun = A.T.matmul; b = Afun(xtrue)
    x = lsmr(A, b)[0]
    residual = b - Afun(x)
